...
void CSerial::checkHeart()
{
    ...
    while (1)
    {
        if (time(0) - last_frame_time > MAX_AGE * 2) // 超过两个心跳包的时间没有收到数据包
        {
            if (!h_serial->available()) // 判断串口是否可用，不可用则删除再连接
            {
                ...
                for (int i = 0; i < MAX_RECONNECT_CNT; i++)
                {
                    ...
                    delete h_serial;
                    h_serial = new serial::Serial(SERIAL_PORT_NAME, 921600, serial::Timeout::simpleTimeout(1000));
                    last_frame_time = time(0);
                    if (h_serial->available())
                    {
                        flag = true;
                        break;
                    }
        ...
        time_t cur_time;
        addAndJdugeInterval(cur_time);
    }
}

void CSerial::run()
{
    std::cout << h_serial->available() << std::endl;
    while (true)
    {
        ...
        case HEAD:
        {
            ...
            {
                if (read_buffer[0] == FRAME_HEAD_0 && read_buffer[1] == FRAME_HEAD_1)
                {
                    read_index = 0;
                    cur_bytes_status = SN;
                }
            ...
            if (h_serial->read(read_buffer, 1) == 1)
            {
                ...
                { // 心跳帧或心跳回复帧
                    ...
                    if (last_hb_sn != -1 && (last_hb_sn + 1) % MAX_SN_CNT != temp_sn && last_hb_sn != temp_sn)
                    {
                        ...
                        if(ids != "") LOG_SOCKET("串口心跳帧 " + ids + " 丢失", LOGGER_ALARM);
                    }
                    last_hb_sn = temp_sn;
                ...
                else if (read_buffer[0] == D_FRAME)
                {
                    ...
                    if (last_data_sn != -1 && (last_data_sn + 1) % MAX_SN_CNT != temp_sn && last_data_sn != temp_sn) // 逻辑同心跳帧丢失
                    {
                        ...
                        if(ids != "") LOG_SOCKET("串口数据帧 " + ids + " 丢失", LOGGER_ALARM);
                    }
        ...
        case TAIL:
        {
            if (h_serial->read(read_buffer, 2) == 2)
            {
                if (read_buffer[0] == FRAME_TAIL_0 && read_buffer[1] == FRAME_TAIL_1)
                {
                    ...
                    { // 数据帧
                        ...
                        case M2O_UD_STATUS: // 3.1 MCU状态汇报，
                        {
                            ...
                            {
                                if(status_type_b0b1 == 0x00) event_number_queue->Push(E2); // 触发E2
                        ...
                        case UD_SHUTTLE_POS:
                        {
                            ...
                            shuttle_pos msg(CRC_buffer[5], colNo, D, Y, cur_timestamp);
...
void CSerial::cwrite_OP10_Frame(unsigned char sn, unsigned char ft, unsigned char dst, unsigned char src, unsigned char *data, unsigned char length)
{
    ...
    for (i = 0; i <= 3; i++)
    {
        if (!flag)
            break;
        cwrite(sn, ft, dst, src, data, length);
        usleep(1000);
        //_mutex2.lock();
        flag = sn_set.find(sn) != sn_set.end();
        //_mutex2.unlock();
    ...
    if (flag)
    {
        ...
        sn_set.erase(sn);
        // TODO：日志记录错误
        LOG_SOCKET("发送帧" + std::to_string(sn) + "失败", LOGGER_ALARM);
    }
}
